...
$canCreate  = $user->authorise('core.create', 'com_location_visits') && file_exists(JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'locationform.xml');
$canEdit    = $user->authorise('core.edit', 'com_location_visits') && file_exists(JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'locationform.xml');
$canCheckin = $user->authorise('core.manage', 'com_location_visits');
$canChange  = $user->authorise('core.edit.state', 'com_location_visits');
$canDelete  = $user->authorise('core.delete', 'com_location_visits');
?>
...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post"
	...
	<?php echo JHtml::_('grid.sort', 'JPUBLISHED', 'a.state', $listDirn, $listOrder); ?>
			...
			<?php endif; ?>
				...
				<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_LOCATION_VISITS_LOCATIONS_STORE', 'a.store', $listDirn, $listOrder); ?>
				</th>
				<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_LOCATION_VISITS_LOCATIONS_CREATED_ON', 'a.created_on', $listDirn, $listOrder); ?>
					...
					<th class="center">
				<?php echo JText::_('COM_LOCATION_VISITS_LOCATIONS_ACTIONS'); ?>
				</th>
				<?php endif; ?>
		...
		<?php foreach ($this->items as $i => $item) : ?>
			<?php $canEdit = $user->authorise('core.edit', 'com_location_visits'); ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) : ?>
					...
					<td class="center">
	<a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canChange) ? JRoute::_('index.php?option=com_location_visits&task=location.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
	<?php if ($item->state == 1): ?>
	...
	<?php if ($canCreate) : ?>
		<a href="<?php echo JRoute::_('index.php?option=com_location_visits&task=locationform.edit&id=0', false, 0); ?>"
		   class="btn btn-success btn-small"><i
				class="icon-plus"></i>
			<?php echo JText::_('COM_LOCATION_VISITS_ADD_ITEM'); ?></a>
	<?php endif; ?>
...
<script type="text/javascript">
	...
	function deleteItem() {
		...
		if (!confirm("<?php echo JText::_('COM_LOCATION_VISITS_DELETE_MESSAGE'); ?>")) {
			return false;
		}
	}
</script>
