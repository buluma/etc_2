...
/**
 * Location_visits model.
 *
 * @since  1.6
 */
class Location_visitsModelLocation extends JModelItem
{
    public $_item;
	...
	{
		$app  = Factory::getApplication('com_location_visits');
		$user = Factory::getUser();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_location_visits')) && (!$user->authorise('core.edit', 'com_location_visits')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_location_visits.edit.location.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_location_visits.edit.location.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new Exception(JText::_('COM_LOCATION_VISITS_ITEM_NOT_LOADED'), 403);
					}
				}
	 ...
	 */
	public function getTable($type = 'Location', $prefix = 'Location_visitsTable', $config = array())
	{
		$this->addTablePath(JPATH_ADMINISTRATOR . '/components/com_location_visits/tables');
