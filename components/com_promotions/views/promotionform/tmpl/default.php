...
$lang->load('com_promotions', JPATH_SITE);
$doc = JFactory::getDocument();
$doc->addScript(JUri::base() . '/media/com_promotions/js/form.js');

$user    = JFactory::getUser();
$canEdit = PromotionsHelpersPromotions::canUserEdit($this->item, $user);


?>

<div class="promotion-edit front-end-edit">
	<?php if (!$canEdit) : ?>
		<h3>
			<?php throw new Exception(JText::_('COM_PROMOTIONS_ERROR_MESSAGE_NOT_AUTHORISED'), 403); ?>
		</h3>
	<?php else : ?>
		<?php if (!empty($this->item->id)): ?>
			<h1><?php echo JText::sprintf('COM_PROMOTIONS_EDIT_ITEM_TITLE', $this->item->id); ?></h1>
		<?php else: ?>
			<h1><?php echo JText::_('COM_PROMOTIONS_ADD_ITEM_TITLE'); ?></h1>
		<?php endif; ?>

		<form id="form-promotion"
			  action="<?php echo JRoute::_('index.php?option=com_promotions&task=promotion.save'); ?>"
	...
	<?php echo $this->form->renderField('server_modified_date'); ?>
			...
			<div class="control-group">
				<div class="controls">
					...
					<?php endif; ?>
					<a class="btn"
					   href="<?php echo JRoute::_('index.php?option=com_promotions&task=promotionform.cancel'); ?>"
			...
			<input type="hidden" name="option" value="com_promotions"/>
			<input type="hidden" name="task"
				   value="promotionform.save"/>
