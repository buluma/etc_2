 ...
 * @since  1.6
 */
class Competitor_activitiesModelComp_activityForm extends JModelForm
{
	private $item = null;
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if ($table !== false && $table->load($id))
			{
				...
				$id   = $table->id;
				$canEdit = $user->authorise('core.edit', 'com_competitor_activities') || $user->authorise('core.create', 'com_competitor_activities');

				if (!$canEdit && $user->authorise('core.edit.own', 'com_competitor_activities'))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_competitor_activities.comp_activity', 'comp_activityform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_competitor_activities.edit.comp_activity.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('comp_activity.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_competitor_activities') || $authorised = $user->authorise('core.edit.own', 'com_competitor_activities');
		}
		else
		{
			// Check the user can create new items in this section
			$authorised = $user->authorise('core.create', 'com_competitor_activities');
		}
	...
	public function delete($pk)
	{
		...
		if (empty($pk))
		{
			$pk = (int) $this->getState('comp_activity.id');
		}

		if ($pk == 0 || $this->getItem($pk) == null)
		{
			throw new Exception(JText::_('COM_COMPETITOR_ACTIVITIES_ITEM_DOESNT_EXIST'), 404);
		}

		if ($user->authorise('core.delete', 'com_competitor_activities') !== true)
		{
			throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
		}
