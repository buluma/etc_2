...
$lang->load('com_tl_objectives', JPATH_SITE);
$doc = JFactory::getDocument();
$doc->addScript(JUri::base() . '/media/com_tl_objectives/js/form.js');

$user    = JFactory::getUser();
$canEdit = Tl_objectivesHelpersTl_objectives::canUserEdit($this->item, $user);


?>

<div class="tl_objective-edit front-end-edit">
	<?php if (!$canEdit) : ?>
		<h3>
			<?php throw new Exception(JText::_('COM_TL_OBJECTIVES_ERROR_MESSAGE_NOT_AUTHORISED'), 403); ?>
		</h3>
	<?php else : ?>
		<?php if (!empty($this->item->id)): ?>
			<h1><?php echo JText::sprintf('COM_TL_OBJECTIVES_EDIT_ITEM_TITLE', $this->item->id); ?></h1>
		<?php else: ?>
			<h1><?php echo JText::_('COM_TL_OBJECTIVES_ADD_ITEM_TITLE'); ?></h1>
		<?php endif; ?>

		<form id="form-tl_objective"
			  action="<?php echo JRoute::_('index.php?option=com_tl_objectives&task=tl_objective.save'); ?>"
	...
	<?php echo $this->form->renderField('user_id'); ?>
			...
			<div class="control-group">
				<div class="controls">
					...
					<?php endif; ?>
					<a class="btn"
					   href="<?php echo JRoute::_('index.php?option=com_tl_objectives&task=tl_objectiveform.cancel'); ?>"
			...
			<input type="hidden" name="option" value="com_tl_objectives"/>
			<input type="hidden" name="task"
				   value="tl_objectiveform.save"/>
