 ...
 * @since  1.6
 */
class Bts_itemsModelBts_itemForm extends JModelForm
{
	private $item = null;
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if ($table !== false && $table->load($id))
			{
				...
				$id   = $table->id;
				$canEdit = $user->authorise('core.edit', 'com_bts_items') || $user->authorise('core.create', 'com_bts_items');

				if (!$canEdit && $user->authorise('core.edit.own', 'com_bts_items'))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_bts_items.bts_item', 'bts_itemform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_bts_items.edit.bts_item.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('bts_item.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_bts_items') || $authorised = $user->authorise('core.edit.own', 'com_bts_items');
		}
		else
		{
			// Check the user can create new items in this section
			$authorised = $user->authorise('core.create', 'com_bts_items');
		}
	...
	public function delete($pk)
	{
		...
		if (empty($pk))
		{
			$pk = (int) $this->getState('bts_item.id');
		}

		if ($pk == 0 || $this->getItem($pk) == null)
		{
			throw new Exception(JText::_('COM_BTS_ITEMS_ITEM_DOESNT_EXIST'), 404);
		}

		if ($user->authorise('core.delete', 'com_bts_items') !== true)
		{
			throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
		}
