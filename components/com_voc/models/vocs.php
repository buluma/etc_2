...
class VocModelVocs extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_voc'))
		{
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->category = JText::_('COM_VOC_VOCS_CATEGORY_OPTION_' . strtoupper($item->category));

			$item->items = JText::_('COM_VOC_VOCS_ITEMS_OPTION_' . strtoupper($item->items));

			$item->commentby = JText::_('COM_VOC_VOCS_COMMENTBY_OPTION_' . strtoupper($item->commentby));
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_VOC_SEARCH_FILTER_DATE_FORMAT"), "warning");
