 ...
 * @since  1.6
 */
class VocModelVoc extends JModelItem
{
    public $_item;
	...
	{
		$app  = Factory::getApplication('com_voc');
		$user = Factory::getUser();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_voc')) && (!$user->authorise('core.edit', 'com_voc')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_voc.edit.voc.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_voc.edit.voc.id', $id);
		}

		$this->setState('voc.id', $id);
		...
		{
			$this->setState('voc.id', $params_array['item_id']);
		}
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new Exception(JText::_('COM_VOC_ITEM_NOT_LOADED'), 403);
					}
				}
		...
		if (!empty($this->_item->category))
		{
			$this->_item->category = JText::_('COM_VOC_VOCS_CATEGORY_OPTION_' . $this->_item->category);
		}

		if (!empty($this->_item->items))
		{
			$this->_item->items = JText::_('COM_VOC_VOCS_ITEMS_OPTION_' . $this->_item->items);
		}

		if (!empty($this->_item->commentby))
		{
			$this->_item->commentby = JText::_('COM_VOC_VOCS_COMMENTBY_OPTION_' . $this->_item->commentby);
		}
	 ...
	 */
	public function getTable($type = 'Voc', $prefix = 'VocTable', $config = array())
	{
		$this->addTablePath(JPATH_ADMINISTRATOR . '/components/com_voc/tables');
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('voc.id');
