 ...
 * @since  1.6
 */
class ObjectivesModelObjectivesForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		{
			$id = Factory::getApplication()->getUserState('com_objectives.edit.objectives.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_objectives.edit.objectives.id', $id);
		}

		$this->setState('objectives.id', $id);
		...
		{
			$this->setState('objectives.id', $params_array['item_id']);
		}
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if (empty($id))
			{
				$id = $this->getState('objectives.id');
			}
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('objectives.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_objectives.objectives', 'objectivesform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_objectives.edit.objectives.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('objectives.id');
	...
	public function delete($pk)
	{
		...
		if (empty($pk))
		{
			$pk = (int) $this->getState('objectives.id');
		}
