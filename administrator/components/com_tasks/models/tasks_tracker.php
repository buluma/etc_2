 ...
 * @since  1.6
 */
class TasksModelTasks_tracker extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering priority
		$this->setState('filter.priority', $app->getUserStateFromRequest($this->context.'.filter.priority', 'filter_priority', '', 'string'));


		// Load the parameters.
		$params = JComponentHelper::getParams('com_tasks');
		$this->setState('params', $params);

		// List state information.
		parent::populateState('a.task', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		if ($filter_deadline_from !== null && !empty($filter_deadline_from))
		{
			$query->where("a.`deadline` >= '".$db->escape($filter_deadline_from)."'");
		}
		$filter_deadline_to = $this->state->get("filter.deadline.to");

		if ($filter_deadline_to !== null  && !empty($filter_deadline_to))
		{
		...
		if ($filter_priority !== null && (is_numeric($filter_priority) || !empty($filter_priority)))
		{
	...
	public function getItems()
	{
		...
		{
					$oneItem->priority = JText::_('COM_TASKS_TASKS_PRIORITY_OPTION_' . strtoupper($oneItem->priority));
		}
