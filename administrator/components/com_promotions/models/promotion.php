 ...
 * @since  1.6
 */
class PromotionsModelPromotion extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_PROMOTIONS';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_promotions.promotion';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_promotions.promotion', 'promotion',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_promotions.edit.promotion.data', array());
	...
	/**
	 * Method to duplicate an Promotion
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_promotions'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__promotions');
				$max             = $db->loadResult();
