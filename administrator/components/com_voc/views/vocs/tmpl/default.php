...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_voc/assets/css/voc.css');
$document->addStyleSheet(JUri::root() . 'media/com_voc/css/list.css');
//datatables
...
$canOrder  = $user->authorise('core.edit.state', 'com_voc');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_voc&task=vocs.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'vocList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}

$sortFields = $this->getSortFields();
?>

<form action="<?php echo JRoute::_('index.php?option=com_voc&view=vocs'); ?>" method="post"
			...
			<table class="table table-striped" id="vocList">
				...
				<tr>
					...
					<?php endif; ?>
									...
									<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_ID', 'a.`id`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_SUBMITTER', 'a.`submitter`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_PHOTOS', 'a.`photos`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_BRAND', 'a.`brand`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_BRANDCODE', 'a.`brandcode`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_CATEGORY', 'a.`category`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_ITEMS', 'a.`items`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_COMMENTBY', 'a.`commentby`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_ISURGENT', 'a.`isurgent`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				...
				<th class='left'>
				<?php echo JHtml::_('searchtools.sort',  'COM_VOC_VOCS_COORDINATES', 'a.`coordinates`', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_voc');
					$canEdit    = $user->authorise('core.edit', 'com_voc');
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'vocs.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
				...
				<td>
					...
					<?php echo JHtml::_('listhelper.toggle', $item->isurgent, 'vocs', 'isurgent', $i); ?>
				</td>				<td>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_voc&task=voc.edit&id='.(int) $item->id); ?>">
