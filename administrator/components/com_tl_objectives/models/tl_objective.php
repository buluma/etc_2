 ...
 * @since  1.6
 */
class Tl_objectivesModelTl_objective extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_TL_OBJECTIVES';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_tl_objectives.tl_objective';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_tl_objectives.tl_objective', 'tl_objective',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_tl_objectives.edit.tl_objective.data', array());
	...
	/**
	 * Method to duplicate an Tl_objective
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_tl_objectives'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__tl_objectives');
				$max             = $db->loadResult();
