 ...
 * @since  1.6
 */
class BrandstocksModelTbrandstock extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_BRANDSTOCKS';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_brandstocks.tbrandstock';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_brandstocks.tbrandstock', 'tbrandstock',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_brandstocks.edit.tbrandstock.data', array());
	...
	/**
	 * Method to duplicate an Tbrandstock
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_brandstocks'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__brandstocks');
				$max             = $db->loadResult();
