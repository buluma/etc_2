...
/**
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering imagepath
		$this->setState('filter.imagepath', $app->getUserStateFromRequest($this->context.'.filter.imagepath', 'filter_imagepath', '', 'string'));

		// Filtering imagedate
		$this->setState('filter.imagedate.from', $app->getUserStateFromRequest($this->context.'.filter.imagedate.from', 'filter_from_imagedate', '', 'string'));
		$this->setState('filter.imagedate.to', $app->getUserStateFromRequest($this->context.'.filter.imagedate.to', 'filter_to_imagedate', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__asset_management` AS a');
		...
		// Filtering imagedate
		$filter_imagedate_from = $this->state->get("filter.imagedate.from");

		if ($filter_imagedate_from !== null && !empty($filter_imagedate_from))
		{
			$query->where("a.`imagedate` >= '".$db->escape($filter_imagedate_from)."'");
		}
		$filter_imagedate_to = $this->state->get("filter.imagedate.to");

		if ($filter_imagedate_to !== null  && !empty($filter_imagedate_to))
		{
			$query->where("a.`imagedate` <= '".$db->escape($filter_imagedate_to)."'");
		}
