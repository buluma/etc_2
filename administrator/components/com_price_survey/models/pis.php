 ...
 * @since  1.6
 */
class Price_surveyModelPis extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_PRICE_SURVEY';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_price_survey.pis';
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_price_survey.edit.pis.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_price_survey'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__price_survey');
				$max             = $db->loadResult();
