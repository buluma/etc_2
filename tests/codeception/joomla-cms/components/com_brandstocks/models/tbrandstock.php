 ...
 * @since  1.6
 */
class BrandstocksModelTbrandstock extends JModelItem
{
    public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_brandstocks')) && (!$user->authorise('core.edit', 'com_brandstocks')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new Exception(JText::_('COM_BRANDSTOCKS_ITEM_NOT_LOADED'), 403);
					}
				}
		...
		if (isset($this->_item->modified_by))
		{
			$this->_item->modified_by_name = Factory::getUser($this->_item->modified_by)->name;
					...
					$this->_item->order_placed = JText::_('COM_BRANDSTOCKS_TBRANDSTOCKS_ORDER_PLACED_OPTION_' . $this->_item->order_placed);
					$this->_item->delivered = JText::_('COM_BRANDSTOCKS_TBRANDSTOCKS_DELIVERED_OPTION_' . $this->_item->delivered);
