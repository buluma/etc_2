 ...
 * @since  1.6
 */
class Price_surveyModelPisForm extends JModelForm
{
	private $item = null;
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if ($table !== false && $table->load($id))
			{
				...
				$id   = $table->id;
				$canEdit = $user->authorise('core.edit', 'com_price_survey') || $user->authorise('core.create', 'com_price_survey');

				if (!$canEdit && $user->authorise('core.edit.own', 'com_price_survey'))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_price_survey.pis', 'pisform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_price_survey.edit.pis.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('pis.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_price_survey') || $authorised = $user->authorise('core.edit.own', 'com_price_survey');
		}
		else
		{
			// Check the user can create new items in this section
			$authorised = $user->authorise('core.create', 'com_price_survey');
		}
	...
	public function delete($pk)
	{
		...
		if (empty($pk))
		{
			$pk = (int) $this->getState('pis.id');
		}

		if ($pk == 0 || $this->getItem($pk) == null)
		{
			throw new Exception(JText::_('COM_PRICE_SURVEY_ITEM_DOESNT_EXIST'), 404);
		}

		if ($user->authorise('core.delete', 'com_price_survey') !== true)
		{
			throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
		}
