 ...
 * @since  1.6
 */
class PhotosModelPhoto extends JModelItem
{
    public $_item;
	...
	{
		$app  = Factory::getApplication('com_photos');
		$user = Factory::getUser();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_photos')) && (!$user->authorise('core.edit', 'com_photos')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_photos.edit.photo.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_photos.edit.photo.id', $id);
		}

		$this->setState('photo.id', $id);
		...
		{
			$this->setState('photo.id', $params_array['item_id']);
		}
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new Exception(JText::_('COM_PHOTOS_ITEM_NOT_LOADED'), 403);
					}
				}
		...
		if (isset($this->_item->store) && $this->_item->store != '')
		{
			...
			{
				$db    = Factory::getDbo();
				$query = "SELECT id as value, shop_name as text FROM dxcr2_outlets HAVING value LIKE '" . $value . "'";
	 ...
	 */
	public function getTable($type = 'Photo', $prefix = 'PhotosTable', $config = array())
	{
		$this->addTablePath(JPATH_ADMINISTRATOR . '/components/com_photos/tables');
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('photo.id');
