...
class AssetsModelAssets extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_assets'))
		{
		...
		$filter_imagedate_from = $this->state->get("filter.imagedate_from_dateformat");
		$filter_Qimagedate_from = (!empty($filter_imagedate_from)) ? $this->isValidDate($filter_imagedate_from) : null;

		if ($filter_Qimagedate_from != null)
		{
			$query->where("a.imagedate >= '" . $db->escape($filter_Qimagedate_from) . "'");
		}

		$filter_imagedate_to = $this->state->get("filter.imagedate_to_dateformat");
		$filter_Qimagedate_to = (!empty($filter_imagedate_to)) ? $this->isValidDate($filter_imagedate_to) : null ;

		if ($filter_Qimagedate_to != null)
		{
			$query->where("a.imagedate <= '" . $db->escape($filter_Qimagedate_to) . "'");
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_ASSETS_SEARCH_FILTER_DATE_FORMAT"), "warning");
