 ...
 * @since  1.6
 */
class VocTablevoc extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'VocTablevoc', array('typeAlias' => 'com_voc.voc'));
		parent::__construct('#__voc', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for multiple field: items
		if (isset($array['items']))
		{
			...
			elseif (strpos($array['items'], ',') != false)
			{
				$array['items'] = explode(',',$array['items']);
			}
			elseif (strlen($array['items']) == 0)
			{
				$array['items'] = '';
			}
		}
		...
		// Support for multiple field: commentby
		if (isset($array['commentby']))
		{
			if (is_array($array['commentby']))
			{
				$array['commentby'] = implode(',',$array['commentby']);
			}
			elseif (strpos($array['commentby'], ',') != false)
			{
				$array['commentby'] = explode(',',$array['commentby']);
			}
			elseif (strlen($array['commentby']) == 0)
			{
				$array['commentby'] = '';
			}
		}
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_voc.voc.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_voc/access.xml',
				"/access/section[@name='voc']/"
			);
			$default_actions = JAccess::getAssetRules('com_voc.voc.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_voc');
