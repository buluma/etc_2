 ...
 * @since  1.6
 */
class VocModelVoc extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_VOC';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_voc.voc';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$form = $this->loadForm(
			...
			if($form->getFieldAttribute('insert_date', 'default') == 'NOW'){
				$form->setFieldAttribute('insert_date', 'default', date('Y-m-d H:i:s'));
			}
		if (empty($form))
		{
			return false;
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_voc.edit.voc.data', array());
		...
		if (empty($data))
		{
			...
			foreach ((array) $data->items as $value)
			{
				if (!is_array($value))
				{
					$array[] = $value;
			...
			// Support for multiple or not foreign key field: commentby
			$array = array();

			foreach ((array) $data->commentby as $value)
			{
				if (!is_array($value))
				{
					$array[] = $value;
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_voc'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__voc');
				$max             = $db->loadResult();
