 ...
 * @since  1.6
 */
class AssetsModelAsset extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_ASSETS';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_assets.asset';
	 ...
	 * @since    1.6
	 */
	public function getTable($type = 'Asset', $prefix = 'AssetsTable', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_assets.edit.asset.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_assets'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__asset_management');
				$max             = $db->loadResult();
