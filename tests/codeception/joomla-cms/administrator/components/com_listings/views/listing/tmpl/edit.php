...
$document->addStyleSheet(JUri::root() . 'media/com_listings/css/form.css');
?>
<script type="text/javascript">
	js = jQuery.noConflict();
	js(document).ready(function () {
		
	js('input:hidden.listing').each(function(){
		var name = js(this).attr('name');
		if(name.indexOf('listinghidden')){
			js('#jform_listing option[value="'+js(this).val()+'"]').attr('selected',true);
		}
	});
	js("#jform_listing").trigger("liszt:updated");
...
<form
	action="<?php echo JRoute::_('index.php?option=com_listings&layout=edit&id=' . (int) $this->item->id); ?>"
	method="post" enctype="multipart/form-data" name="adminForm" id="listing-form" class="form-validate">

	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_LISTINGS_TITLE_LISTING', true)); ?>
		<div class="row-fluid">
			<div class="span10 form-horizontal">
				<fieldset class="adminform">

									<input type="hidden" name="jform[id]" value="<?php echo $this->item->id; ?>" />
				<?php echo $this->form->renderField('listing'); ?>

			<?php
				foreach((array)$this->item->listing as $value): 
					if(!is_array($value)):
						echo '<input type="hidden" class="listing" name="jform[listinghidden]['.$value.']" value="'.$value.'" />';
					endif;
				endforeach;
			?>				<input type="hidden" name="jform[ordering]" value="<?php echo $this->item->ordering; ?>" />
				...
				<?php echo $this->form->renderField('listed'); ?>
