 ...
 * @since  1.6
 */
class ListingsTablelisting extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'ListingsTablelisting', array('typeAlias' => 'com_listings.listing'));
		parent::__construct('#__listings', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for multiple field: listing
		if (isset($array['listing']))
		{
			if (is_array($array['listing']))
			{
				$array['listing'] = implode(',',$array['listing']);
			}
			elseif (strpos($array['listing'], ',') != false)
			{
				$array['listing'] = explode(',',$array['listing']);
			}
			elseif (strlen($array['listing']) == 0)
			{
				$array['listing'] = '';
			}
		}
		...
		if ($task == 'apply' || $task == 'save')
		{
			$array['server_modified_date'] = $date->toSql();
		}
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_listings.listing.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_listings/access.xml',
				"/access/section[@name='listing']/"
			);
			$default_actions = JAccess::getAssetRules('com_listings.listing.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_listings.listing.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_listings');
