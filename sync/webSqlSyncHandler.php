...
final class SqlSyncHandler{
	...
	public function __construct(){					
		...
		//$this->jsonData = file_get_contents('jsondata/1458415013_syncdata.json');		// json file to string 
		...
		// read clientdata and get the username in info
		$sync_info = $this->clientData->info[0];
		$username = $sync_info->user_name;
		// save Client JSON data to a folder with their username
		// May 05, 2017 @eric
		if (isset($username)){
		    // test the directory first
		    if (!is_dir('jsondata/' . $username)) {
              // if user directory doesn't exist, make it
              mkdir('jsondata/' . $username);
            }
		    file_put_contents('jsondata/'.$username.'/'.$mytime.'_syncdata.json', $this->jsonData);
		}
		else {
